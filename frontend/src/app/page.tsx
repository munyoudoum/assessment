          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-2xl border border-white/20 overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 px-4 sm:px-8 py-4 sm:py-6 relative overflow-hidden">
              ...
              <div className="relative">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    <h1 className="text-3xl sm:text-3xl font-bold text-white mb-1 sm:mb-2 tracking-tight">
                      ✨ My Tasks
                    </h1>
                    <p className="text-indigo-100 text-sm sm:text-sm font-medium">
                      Stay organized and productive
                    </p>
                  </div>
                  {totalCount > 0 && (
                    <div className="text-left sm:text-right">
                      <div className="text-white/90 text-sm sm:text-sm font-medium mb-1">
                        Progress
                      </div>
                      <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                        ...
                        <span className="text-white font-bold text-lg sm:text-lg whitespace-nowrap">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full px-4 sm:px-6 py-3 sm:py-4 text-lg sm:text-lg text-gray-800 border-2 border-gray-200 rounded-xl sm:rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 bg-gray-50 hover:bg-white hover:shadow-md placeholder-gray-400"
                  />
                </div>
                <button
                  onClick={createTodo}
                  disabled={!newTodoTitle.trim() || loading}
                  className="px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl sm:rounded-2xl hover:from-indigo-700 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-semibold text-lg sm:text-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 active:translate-y-0 w-full sm:w-auto"
                >
                  Add Task
                </button>
              </div>
            ...
            {error && (
              <div className="mx-4 sm:mx-8 mt-4 sm:mt-6 p-3 sm:p-4 bg-red-50 border-l-4 border-red-400 rounded-r-lg animate-in slide-in-from-top duration-300">
                <div className="flex items-center">
                  ...
                  <p className="text-red-700 font-medium text-base sm:text-base">{error}</p>
                </div>
              </div>
            )}
            ...
            {loading && (
              <div className="px-4 sm:px-8 py-8 sm:py-12 text-center">
                ...
                <p className="mt-3 sm:mt-4 text-gray-600 font-medium text-base sm:text-base">Loading your tasks...</p>
              </div>
            )}
                ...
                <div className="max-h-80 sm:max-h-96 overflow-y-auto">
                  {todos.length === 0 ? (
                    <div className="px-4 sm:px-8 py-8 sm:py-12 text-center">
                      ...
                      <h3 className="text-xl sm:text-xl font-semibold text-gray-700 mb-2">No tasks yet</h3>
                      <p className="text-gray-500 text-base sm:text-base">Add your first task above to get started!</p>
                    </div>
                  ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            {editingId === todo.id ? (
                              <input
                                ...
                                autoFocus
                                className="w-full px-3 sm:px-4 py-2 text-lg sm:text-lg text-gray-800 border-2 border-indigo-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-white"
                              />
                            ) : (
                              <span
                                onClick={() => startEdit(todo)}
                                className={`block cursor-pointer text-lg sm:text-lg transition-all duration-200 ${
                                  todo.completed
            ...
            {!loading && todos.length > 0 && (
              <div className="px-4 sm:px-8 py-4 sm:py-6 bg-gradient-to-r from-gray-50 to-indigo-50 border-t border-gray-100">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
                  <div className="flex flex-wrap items-center gap-3 sm:gap-6 text-sm sm:text-sm">
                  ...
                  {completedCount > 0 && (
                    <button
                      ...
                      className="text-red-600 hover:text-red-800 font-medium hover:bg-red-50 px-3 py-1.5 sm:py-1 rounded-lg transition-colors duration-200 text-sm sm:text-sm w-full sm:w-auto text-center"
                    >
                      Clear completed
                    </button>
                  )}
